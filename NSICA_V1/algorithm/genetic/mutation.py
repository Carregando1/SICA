...
class ArbitraryRulesetMutator(Mutator):
    ...
    def init_state(self) -> CurrentState:
        ...
        rules = jnp.zeros((self.grid_size - 1, self.grid_size, self.grid_size, self.states * math.comb(7+self.states, 8)))
        initial = jax.random.choice(
            key=jax.random.key(random.randint(0,2147483648)),
            a = range(self.states),
    ...
    def mutate(self, state: CurrentState) -> tuple[CurrentState, MutationSet]:
        ...
        #Mutate initials.
        if (self.ic_enable):
            new_state.initial = state.initial + ((jax.random.uniform(jax.random.key(random.randint(0,2147483648)), state.initial.shape[0],state.initial.shape[1]) < self.mutate_p)*jax.random.choice(jax.random.key(random.randint(0,2147483648)),  self.states))
            new_state.initial = new_state.initial%self.states
            a = jnp.nonzero(new_state.initial != state.initial)
            mutations.ic_mutations = jnp.concatenate((jnp.transpose(a), state.initial[a].reshape(-1,1), new_state.initial[a].reshape(-1,1)), axis=1).tolist()
        ...
        if (self.srt_enable):
            if (self.strict): 
                ...
                new_state.rules = state.rules + (jax.random.uniform(jax.random.key(random.randint(0,2147483648)), state.rules.shape[0], state.rules.shape[1], state.rules.shape[2], 1) < self.mutate_p) * (jax.random.uniform(jax.random.key(random.randint(0,2147483648)), state.rules.shape[0], state.rules.shape[1], state.rules.shape[2], state.rules.shape[3]) < self.rule_mutate_p) * jax.random.choice(jax.random.key(random.randint(0,2147483648)), self.states, size=state.rules.shape)
            else:
                #Selects arbitrary SRT cells with probability rule_mutate_p
                new_state.rules = state.rules + (jax.random.uniform(jax.random.key(random.randint(0,2147483648)), state.rules.shape[0],state.rules.shape[1],state.rules.shape[2],state.rules.shape[3]) < self.rule_mutate_p)*jax.random.choice(jax.random.key(random.randint(0,2147483648)), self.states, size=state.rules.shape)
            new_state.rules = new_state.rules%self.states
            a = jnp.nonzero(new_state.rules != state.rules)
            mutations.srt_mutations = jnp.concatenate((jnp.transpose(a), state.rules[a].reshape(-1,1), new_state.rules[a].reshape(-1,1)), axis=1).tolist()
