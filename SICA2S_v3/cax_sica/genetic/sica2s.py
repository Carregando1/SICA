...
class SICA2SUpdate(Update):
	...
	def update_srt(self, srt) -> None:
		"""Update the SRT of a 2-state SICA from an array input.
...
class SICA2S(CA):
	...
	def __init__(self, time, rngs: nnx.Rngs, *, metrics_fn: Callable = metrics_fn):
		...
		perceive = SICA2SPerceive(rngs=rngs)
		update = SICA2SUpdate(time=time, rngs=rngs)
